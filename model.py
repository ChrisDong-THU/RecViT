...
class VisionTransformer(nn.Module):
    def __init__(self, *, image_size, patch_size, dim, depth, heads, mlp_dim, pool = 'mean', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        Args:
            image_size (tuple): 图片大小
            patch_size (tuple): 块大小,patch一般为正方形
            dim (int): Transformer所有层潜在向量的大小
            depth (int): Transformer内部层数,取12
            heads (_type_): 注意力机制中的头数,取8
            mlp_dim (_type_): MLP隐藏层维度
            dim_head: 注意力头的数据维度
            dropout: 位置编码后dropout
            emb_dropout: 注意力dropout
        '''
        super().__init__()
    ...
    def autoencoder(self, x):
        ...
        # cls_token所对应的输出向量将被用于分类任务，TODO: 改成适用图像复原任务
...
class RecFieldViT(nn.Module):
    def __init__(self,
        ...
        self.num_patch = (image_height // self.patch_size, image_width // self.patch_size)
        base_cfg = dict(
        ...
        output_dim = 1 * patch_size * patch_size
    ...
    def forward(self, x):
        ...
        image_norm_token = image_norm_token.view(-1, self.num_patch[0], self.num_patch[1], dim).permute(0, 3, 2, 1) # 变换维度顺序0321或0312
